...
def construct_hess_flux_matrix_with_bounds(binning_geometry:GammaBinning=None,
        energy_axis: np.ndarray=None, longitudeaxis: np.ndarray=None, latitudeaxis: np.ndarray=None,
        min_abs_lon=0*u.deg, max_abs_lon=20*u.deg,
        min_abs_lat=0*u.deg, max_abs_lat=20*u.deg,
        ):
    """
    ...
    for idx, model in enumerate(hess_models):
        ...
        abs_lon_condition = np.logical_and(np.abs(temp_l_value)>=min_abs_lon, np.abs(temp_l_value)<max_abs_lon)
        abs_lat_condition = np.logical_and(np.abs(temp_b_value)>=min_abs_lat, np.abs(temp_b_value)<max_abs_lat)

        # if lon_include_condition and lat_include_condition:
        #     print(temp_l_value, temp_b_value,)
        #     print(np.abs(temp_l_value), np.abs(temp_b_value))
        #     print(min_abs_lon, min_abs_lat, max_abs_lon, max_abs_lat)
        #     print(abs_lon_condition, abs_lat_condition)

        # If the location of the source satisfies the criteria above then we start to store it's flux values
        if  lon_include_condition and lat_include_condition and abs_lon_condition and abs_lat_condition:
...
class construct_hess_source_map_interpolation_with_bounds(object):
    """
    ...
    def __init__(self,
        ...
        log_astro_sourcemap = np.log(
            construct_hess_flux_matrix_with_bounds(
                binning_geometry=self.binning_geometry,
                min_abs_lon=min_abs_lon, max_abs_lon=max_abs_lon,
                min_abs_lat=min_abs_lat, max_abs_lat=max_abs_lat,
                ).to(hess_flux_units).value)
...
class HESSCatalogueSources_Prior_WithBounds(SourceFluxDiscreteLogPrior):
    """
    ...
    def __init__(self, 
        ...
        self.log_hess_class_instance = construct_hess_source_map_interpolation_with_bounds(
                binning_geometry=self.binning_geometry,
                min_abs_lon=min_abs_lon, max_abs_lon=max_abs_lon,
                min_abs_lat=min_abs_lat, max_abs_lat=max_abs_lat,
            )
